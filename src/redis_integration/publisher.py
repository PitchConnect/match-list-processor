...
class MatchProcessorRedisPublisher:
    ...
    def __init__(self, redis_config: Optional[RedisConnectionConfig] = None) -> None:
        ...
        logger.info("📡 Match Processor Redis Publisher initialized")
    ...
    def publish_match_updates(
        self, matches: List[Dict[str, Any]], changes: Dict[str, Any]
    ) -> PublishResult:
        """
        ...
        try:
            ...
            logger.info(
                f"   Changes: {len(changes.get('new_matches', {}))} new, {len(changes.get('updated_matches', {}))} updated"
            )
            ...
            if subscribers >= 0:
                self.publish_stats["successful_publishes"] = (
                    self.publish_stats.get("successful_publishes", 0) or 0
                ) + 1
                ...
                self.publish_stats["failed_publishes"] = (
                    self.publish_stats.get("failed_publishes", 0) or 0
                ) + 1
            ...
            return PublishResult(
                success=False, channel=channel, subscribers_notified=0, error=error_msg
            )
        finally:
            self.publish_stats["total_published"] = (
                self.publish_stats.get("total_published", 0) or 0
            ) + 1
