...
class PersistentMatchListProcessorApp:
    ...
    def _process_new_matches(self, new_ids: set, current_matches: MatchDict_Dict) -> None:
        ...
        if new_ids:
            ...
            # Create calendar events for all new matches
            self._create_calendar_events(new_ids, current_matches)

            # Process individual matches for WhatsApp avatars
    ...
    def _create_calendar_events(self, match_ids: set, matches: MatchDict_Dict) -> None:
        """Create calendar events for new matches."""
        logger.info(f"Creating calendar events for {len(match_ids)} new matches...")
        ...
        try:
            ...
            calendar_events = []
            target_calendar_id = "e8835809c7a887d4b3138f8b1f3ebc196bd3916dcf0b689b7f8b4a399ffc12df@group.calendar.google.com"
            ...
            for match_id in match_ids:
                ...
                match_time = match_data.get('avsparkstid', '15:00')
                ...
                try:
                    ...
                    if len(date_parts) == 3:
                        ...
                        if len(time_parts) == 2:
                            ...
                            end_time = start_time + timedelta(hours=2)  # 2-hour match duration

                            # Build referee details string (original format)
                            referees_details = []
                            for referee in match_data.get('domaruppdraglista', []):
                                details = f"{referee.get('domarrollkortnamn', referee.get('uppdragstyp', 'Referee'))}:\n"
                                details += f"{referee.get('personnamn', referee.get('namn', 'Unknown'))}\n"
                                if referee.get('mobiltelefon'):
                                    details += f"Mobil: {referee.get('mobiltelefon')}\n"
                                elif referee.get('telefonnummer'):
                                    details += f"Mobil: {referee.get('telefonnummer')}\n"
                                if referee.get('adress') and referee.get('postnr') and referee.get('postort'):
                                    details += f"{referee.get('adress')}, {referee.get('postnr')} {referee.get('postort')}\n"
                                referees_details.append(details)
                            referees_string = "\n".join(referees_details)

                            # Build team contacts string (original format)
                            contact_details = []
                            if match_data.get('kontaktpersoner'):
                                for contact in match_data.get('kontaktpersoner', []):
                                    contact_string = f"{contact.get('lagnamn', 'Team')}:\n"
                                    contact_string += f"Name: {contact.get('personnamn', 'Unknown')}\n"
                                    if contact.get('telefon'):
                                        contact_string += f"Tel: {contact.get('telefon')}\n"
                                    if contact.get('mobiltelefon'):
                                        contact_string += f"Mobil: {contact.get('mobiltelefon')}\n"
                                    if contact.get('epostadress'):
                                        contact_string += f"Email: {contact.get('epostadress')}\n"
                                    contact_details.append(contact_string)
                            contact_string_for_description = "\n".join(contact_details)

                            # Build description (original format)
                            description = f"{match_data.get('matchnr', match_id)}\n"
                            description += f"{match_data.get('tavlingsnamn', 'Unknown Competition')}\n\n"
                            description += f"{referees_string}\n\n"
                            if contact_string_for_description:
                                description += f"Team Contacts:\n{contact_string_for_description}\n\n"
                            description += f"Match Details: https://www.svenskfotboll.se/matchfakta/{match_id}/\n"

                            # Create event data (original format)
                            event_data = {
                                'match_id': match_id,
                                'summary': f"{match_data.get('lag1namn', 'Team 1')} - {match_data.get('lag2namn', 'Team 2')}",
                                'description': description,
                                'location': match_data.get('arenans_namn', 'Unknown Venue'),
                                'start_time': start_time.isoformat(),
                                'end_time': end_time.isoformat(),
                            }

                            calendar_events.append(event_data)
                            logger.info(f"  📅 Prepared calendar event for Match {match_id}: {event_data['summary']}")

                except (ValueError, IndexError) as e:
                    logger.error(f"  ❌ Error parsing date/time for match {match_id}: {e}")
                    continue

            # Send calendar events to calendar sync service
            if calendar_events:
                calendar_sync_url = os.environ.get('CALENDAR_SYNC_URL', 'http://fogis-calendar-phonebook-sync:5003/sync')
                calendar_create_url = calendar_sync_url.replace('/sync', '/create_events')
                ...
                logger.info(f"  🔗 Sending {len(calendar_events)} events to calendar service: {calendar_create_url}")

                response = requests.post(calendar_create_url, json=payload, timeout=60)

                if response.status_code == 200:
                    result = response.json()
                    logger.info(f"  ✅ Calendar events created successfully: {result}")

                    # Log individual event links if available
                    if 'events' in result:
                ...
                logger.warning("  ⚠️ No valid calendar events to create")

        except Exception as e:
            logger.error(f"Error creating calendar events: {e}")
            logger.exception("Calendar event creation stack trace:")

    def _format_referee_list(self, referee_list):
        """Format referee list for calendar event description."""
        if not referee_list:
            return "- No referees assigned"

        formatted = []
        for i, referee in enumerate(referee_list, 1):
            name = referee.get('namn', 'Unknown')
            role = referee.get('uppdragstyp', 'Referee')
