...
class Settings(BaseSettings):
    ...
    fogis_api_client_url: str = Field(
        default="http://fogis-api-client-service:8080", env="FOGIS_API_CLIENT_URL"
    )
    whatsapp_avatar_service_url: str = Field(
        default="http://whatsapp-avatar-service:5002", env="WHATSAPP_AVATAR_SERVICE_URL"
    )
    google_drive_service_url: str = Field(
        default="http://google-drive-service:5000", env="GOOGLE_DRIVE_SERVICE_URL"
    )
    phonebook_sync_service_url: str = Field(
        default="http://fogis-calendar-phonebook-sync:5003", env="PHONEBOOK_SYNC_SERVICE_URL"
    )
    ...
    log_format: str = Field(default="%(asctime)s - %(levelname)s - %(message)s", env="LOG_FORMAT")
